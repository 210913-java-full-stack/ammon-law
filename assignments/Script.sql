...
###########################################################
DROP DATABASE IF EXISTS Ammon_DB;
CREATE DATABASE Ammon_DB;
...
SELECT SUM(balance) AS "Smith Deposits"
FROM accounts a
...
#SELECT c.name, a.balance, ad.state 
UPDATE accounts a 
#FROM accounts a
JOIN accounts_customers ac ON a.account_id = ac.account_id
...
#Transfer not sure how to pick one account of jason smith with only jason smith
...
SELECT REPLACE (c.name, 'Smith', 'LastName')
FROM customers c 
WHERE c.name LIKE '%Amanda Smith%';
